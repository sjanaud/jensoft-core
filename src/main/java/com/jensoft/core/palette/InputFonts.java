...
/**
 * <code>InputFonts</code> is a cache of the dynamically loaded fonts found in the fonts directory.
 * 
...
public class InputFonts {
    ...
    private static String[] names = { ELEMENT, PTF_NORDIC, FUSION, NEUROPOL,
            NO_MOVE, YORKVILLE, SANSATION_REGULAR, SANSATION_BOLD,
            SANSATION_LIGHT, ROADWAY, N_GAGE, MODE_NINE, FATSANS, STASMIC,
            STREET, STREET_ITALIC, SUPREME, X_SCALE, XIROD, XOIREQE, ZRNIC,
            MAC_TYPE, PHOENIX, SAXMONO
    ...
    public static Font getSansation(int size) {
        return getFont("Sansation_Regular.ttf").deriveFont(
                                                           AffineTransform.getScaleInstance(size, size));

    }

    public static Font getElements(int size) {
        return getFont("Elements.ttf").deriveFont(
                                                  AffineTransform.getScaleInstance(size, size));

    }

    public static Font getYorkville(int size) {
        return getFont("yorkville.ttf").deriveFont(
                                                   AffineTransform.getScaleInstance(size, size));

    }

    public static Font getNoMove(int size) {
        return getFont("No-move.ttf").deriveFont(
                                                 AffineTransform.getScaleInstance(size, size));

    }

    public static Font getNeuropol(int size) {
        return getFont("NEUROPOL.ttf").deriveFont(
                                                  AffineTransform.getScaleInstance(size, size));
    }

    public static Font getPTFNordic(int size) {
        return getFont("PTF-NORDIC-Rnd.ttf").deriveFont(
                                                        AffineTransform.getScaleInstance(size, size));
    ...
    public static Font getFont(String name, int size) {
        ...
        if (cache != null) {
            if ((font = cache.get(name)) != null) {
                return font.deriveFont(AffineTransform.getScaleInstance(size,
                                                                        size));
            }
        }
