...
/**
 * <p>
 * A convenience class from which to extend all non-visual AbstractBeans. It manages the PropertyChange notification
 * system, making it relatively trivial to add support for property change events in getters/setters.
 * </p>
 * <p>
 * A non-visual java bean is a Java class that conforms to the AbstractBean patterns to allow visual manipulation of the
 * bean's properties and event handlers at design-time.
 * </p>
 * <p>
 * Here is a simple example bean that contains one property, foo, and the proper pattern for implementing property
 * change notification:
 * 
 * <pre>
 * <code>
 ...
 * </p>
 * <p>
 * You will notice that "getFoo()" is used in the setFoo method rather than accessing "foo" directly for the gets. This
 * is done intentionally so that if a subclass overrides getFoo() to return, for instance, a constant value the property
 * change notification system will continue to work properly.
 * </p>
 * <p>
 * The firePropertyChange method takes into account the old value and the new value. Only if the two differ will it fire
 * a property change event. So you can be assured from the above code fragment that a property change event will only
 * occur if old is indeed different from getFoo()
 * </p>
 * <p>
 * <code>AbstractBean</code> also supports {@link VetoablePropertyChange} events. These events are similar to
 * <code>PropertyChange</code> events, except a special exception can be used to veto changing the property. For
 * example, perhaps the property is changing from "fred" to "red", but a listener deems that "red" is unexceptable. In
 * this case, the listener can fire a veto exception and the property must remain "fred". For example:
 * 
 * <pre>
 * <code>
 *  public class ABean extends AbstractBean {
...
public abstract class AbstractBean {
    ...
    /**
     * Creates a new instance of AbstractBean, using the supplied
     ...
     * than once, and will be called as many times as it is added. If <code>listener</code> is null, no exception is
     * thrown and no action is
     * taken.
     * 
     * @param listener
     ...
     * PropertyChangeListener that was registered for all properties. If <code>listener</code> was added more than once
     * to the same event source,
     * it will be notified one less time after being removed. If <code>listener</code> is null, or was never added, no
     * exception is thrown
     ...
     * PropertyChangeListeners and <code>PropertyChangeListenerProxy</code>s. If the calling method is interested in
     ...
     * for that property. If <code>propertyName</code> or <code>listener</code> is null, no exception is thrown and no
     * action is taken.
     * 
     * @param propertyName
     ...
     *         the named property. If no such listeners have been added, or if <code>propertyName</code> is null, an
     ...
     * {@code PropertyChangeEvent} value.
     * 
     * @param propertyName
     ...
     * and will be called as many times as it is added. If <code>listener</code> is null, no exception is thrown and no
     * action is taken.
     * 
     * @param listener
     ...
     * VetoableChangeListener that was registered for all properties. If <code>listener</code> was added more than once
     * to the same event source,
     * it will be notified one less time after being removed. If <code>listener</code> is null, or was never added, no
     * exception is thrown
     * and no action is taken.
    ...
    /**
     * Remove a VetoableChangeListener for a specific property. If <code>listener</code> was added more than once to the
     * same event source
     * for the specified property, it will be notified one less time after being
     * removed. If <code>propertyName</code> is null, no exception is thrown and
     * no action is taken. If <code>listener</code> is null, or was never added
     ...
     *         named property. If no such listeners have been added, or if <code>propertyName</code> is null, an empty
     *         array is returned.
     */
    public final VetoableChangeListener[] getVetoableChangeListeners(
            String propertyName) {
