...
/**
 * <code>GeneralMetricsPath</code>
 * <br>
 * 
 * <h4>Usage<h4>GeneralMetricsPath which draws path with metrics text<br>
 * with various label and appealing rendering.<br>
 * Application Domain : Gauge <h4>Description<h4>The general metrics path interpretation and concept understanding
 * should be defined by :<br>
 * A path which is defines by a geometry and the path represent locally metrics values along the path between the
 * defined dimension min and a max value which are set.You can add along some label and text for reference metrics you
 * wish, render between min and max value proportional related to the given metric value.
 * <br><br>
 * <h4>Properties</h4> properties summary for GeneralMetricsPath:<br>
 * <br>
 * <p>
 * (1) - Geometry the path is a built-in path that can be built with base geometry such as arc, cubic curve, quadratic
 * curve, line or any another shape geometry which are append. The path projection which is set takes the responsibility
 * to transform path in the desired available coordinate system.
 * </p>
 * <p>
 ...
 * local and user<br>
 * <br>
 * Local Path Projection {@link AbstractMetricsPath.ProjectionNature#DEVICE} process and solve<br>
 * geometry in the component system coordinate and then path is view a non-vector metrics<br>
 * path and then can not support use full related object such a zoom plug in or translate<br>
 * plug in which work in the user coordinate support by window.<br>
 * <br>
 * <br>
 * User Path Projection {@link AbstractMetricsPath.ProjectionNature#DEVICE} process and solve <br>
 * geometry in the user system coordinate and then this processing mode make possible<br>
 * to obtain vector metrics path.<br>
 * </p>
 * <p>
 * (3) - Path Dimension min and max should be defined to assign a scalar dimension to this path. the path has a length
 * and then path can graduate with metrics call {@link GlyphMetric} that represents label and text annotations along the
 * path.
 * </p>
 * <p>
 * (4) - You should add some {@link GlyphMetric} on the path and choosing some rendering properties and rendering
 * strategy (reverse glyph for example).
 * </p>
 * <h4>Path Building</h4>
 * <p>
 * Path building step:<br>
 * (1)create a path and choose projection. (2)define segment like arc, lines, cubic, quadratic other shapes and append
 * to the path, see {@link #append(Shape)} or {@link #moveTo(double, double)} that add segment entry to the path.
 * (3)define min and max dimension value for the path. (4)create metrics and add metrics glyph
 * {@link #addMetric(GlyphMetric)} to path. (5)solved metrics geometry should supplied by {@link #getMetrics()} method
 * moreover developer can be get the radial point to specified metrics value
 * on a path with method {@link #getRadialPoint(double, int, Side)}
 * </p>
 * <p>
 * {@link GeneralMetricsPath} gives a good accuracy to draw gauge path as shown. <center><img
 * src="doc-files/white-tachometer.png"></center><br>
...
public class GeneralMetricsPath extends AbstractMetricsPath {
     ...
     * compass for example, or gauge.
     * you should be make you idea in trying effect by some demo where general
     * path is used, on gauge for example.
     * 
     ...
     * specified x y coordinate, with the control point cx1 cy1 and cx2, cy2 coordinates
     * 
     * @param cx1
     * @param cy1
     * @param cx2
     ...
     * @param y
     */
    public void curveTo(double cx1, double cy1, double cx2, double cy2,
            double x, double y) {
    ...
    public class ShapeGeometry extends SegmentEntry {
         ...
         * @param nature
         */
        public void validCurentSegment(PathIterator pi, GeneralPath path,
                ProjectionNature nature) {
            ...
            switch (type) {
                ...
                case PathIterator.SEG_MOVETO:
                    ...
                    if (getProjectionNature() == ProjectionNature.USER) {
                        pm = getWindow2d().userToPixel(
                                                       new Point2D.Double(coordinates[0], coordinates[1]));
                    }
                    else {
                ...
                case PathIterator.SEG_QUADTO:
                    ...
                    Point2D pq2;
                    if (getProjectionNature() == ProjectionNature.USER) {
                        pq1 = getWindow2d().userToPixel(
                                                        new Point2D.Double(coordinates[0], coordinates[1]));
                        pq2 = getWindow2d().userToPixel(
                                                        new Point2D.Double(coordinates[2], coordinates[3]));
                    }
                    else {
                ...
                case PathIterator.SEG_CUBICTO:
                    ...
                    Point2D pc3;
                    if (getProjectionNature() == ProjectionNature.USER) {
                        pc1 = getWindow2d().userToPixel(
                                                        new Point2D.Double(coordinates[0], coordinates[1]));
                        pc2 = getWindow2d().userToPixel(
                                                        new Point2D.Double(coordinates[2], coordinates[3]));
                        pc3 = getWindow2d().userToPixel(
                                                        new Point2D.Double(coordinates[4], coordinates[5]));
                    }
                    else {
                    ...
                    path.curveTo(pc1.getX(), pc1.getY(), pc2.getX(), pc2.getY(),
                                 pc3.getX(), pc3.getY());
                    break;
    ...
    public class QuadTo extends SegmentEntry {
         ...
         * 
         * @param xcontrol
         *            the x control point coordinate of this quadratic entry to set
         * @param ycontrol
         *            the y control point coordinate of this quadratic entry to set
         * @param x
        ...
        protected void validEntry(GeneralPath path) {
            if (getProjectionNature() == ProjectionNature.USER) {
                Point2D p2dDeviceControl = getWindow2d().userToPixel(
                                                                     new Point2D.Double(xcontrol, ycontrol));
                Point2D p2dDevice = getWindow2d().userToPixel(
                                                              new Point2D.Double(x, y));

                path.quadTo(p2dDeviceControl.getX(), p2dDeviceControl.getY(),
    ...
    public class CurveTo extends SegmentEntry {
         ...
         * 
         * @param xcontrol1
         *            the x coordinate for the first control of this cubic segment entry to set
         * @param ycontrol1
         *            the y coordinate for the first control of this cubic segment entry to set
         * @param xcontrol2
         *            the x coordinate for the second control of this cubic segment entry to set
         * @param ycontrol2
         *            the y coordinate for the second control of this cubic segment entry to set
         * @param x
         *            the x coordinate for this cubic segment entry to set
         * @param y
         *            the x coordinate for this cubic segment entry to set
         */
        public CurveTo(double xcontrol1, double ycontrol1, double xcontrol2,
                double ycontrol2, double x, double y) {
        ...
        protected void validEntry(GeneralPath path) {
            if (getProjectionNature() == ProjectionNature.USER) {
                Point2D p2dDeviceControl1 = getWindow2d().userToPixel(
                                                                      new Point2D.Double(xcontrol1, ycontrol1));
                Point2D p2dDeviceControl2 = getWindow2d().userToPixel(
                                                                      new Point2D.Double(xcontrol2, ycontrol2));
                Point2D p2dDevice = getWindow2d().userToPixel(
                                                              new Point2D.Double(x, y));

                path.curveTo(p2dDeviceControl1.getX(),
                             p2dDeviceControl1.getY(), p2dDeviceControl2.getX(),
                             p2dDeviceControl2.getY(), p2dDevice.getX(),
                             p2dDevice.getY());
            }
            else {
    ...
    /**
     * scale the manager between two space and assign delegate super geometry path for 
     * all method that have to use geometry. 
     */
    ...
    public GlyphMetric solveMetrics(GlyphMetric glyphMetrics) {
        ...
        glyphMetrics.setMetricGlyphMarker(new Marker(geometry
        ...
        GlyphVector glyphVector = f.createGlyphVector(getFontRenderContext(),
                                                      glyphMetrics.getMetricsLabel());
        ...
        if (glyphMetrics.getStylePosition() == StylePosition.Tangent) {
            ...
            for (int j = 0; j < glyphVector.getNumGlyphs(); j++) {
                ...
                if (!needRevert) {
                    ...
                    pointGlyph = geometry.pointAtLength(endLength
                            - GlyphUtil.getGlyphWidthAtToken(glyphVector, j));
                }
                ...
                GlyphGeometry metricGlyphGeometry = new GlyphGeometry(
                                                                      glyphTransformed, dstNorth, dstSouth, dstWest,
                                                                      dstEast);
        ...
        if (glyphMetrics.getStylePosition() == StylePosition.Radial) {
            ...
            Point2D pEnd = glyphMetrics
                    .getRadialPoint((int) (glyphMetrics.getDivergence() + gvWidth + 10));
            ...
            if (pStart.getX() > pEnd.getX()) {
                radialFragment = new Line2D.Double(pEnd.getX(), pEnd.getY(),
            ...
            for (int j = 0; j < glyphVector.getNumGlyphs(); j++) {
                ...
                Point2D pointGlyph = geometryRadialpath.pointAtLength(GlyphUtil
        ...
        if (glyphMetrics.getStylePosition() == StylePosition.Default) {
            ...
            Point2D pStart = new Point2D.Double(pRadial.getX() - gvWidth / 2,
                                                pRadial.getY());
            Point2D pEnd = new Point2D.Double(pRadial.getX() + gvWidth / 2,
                                              pRadial.getY());
        ...
        for (GlyphMetric vm : volatileMetrics) {
            ...
            if (m.getStylePosition() == StylePosition.Tangent) {
                ...
                for (int j = 0; j < glyphVector.getNumGlyphs(); j++) {
                    ...
                    Point2D srcEast = new Point2D.Double(glyph.getBounds2D()
                            .getX() + glyph.getBounds2D().getWidth(), glyph
                            .getBounds2D().getCenterY());
                    Point2D dstEast = new Point2D.Double();

                    Point2D srcWest = new Point2D.Double(glyph.getBounds2D()
                            .getX(), glyph.getBounds2D().getCenterY());
                    Point2D dstWest = new Point2D.Double();
                    ...
                    GlyphGeometry metricGlyphGeometry = new GlyphGeometry(
                                                                          glyphTransformed, dstNorth, dstSouth,
                                                                          dstWest,
                                                                          dstEast);
            ...
            if (m.getStylePosition() == StylePosition.Radial) {
                ...
                for (int j = 0; j < glyphVector.getNumGlyphs(); j++) {
                    ...
                    Point2D pointGlyph = geometryRadialpath
            ...
            if (m.getStylePosition() == StylePosition.Default) {
                ...
                Point2D pStart = new Point2D.Double(pRadial.getX() - gvWidth
                        / 2, pRadial.getY());
                Point2D pEnd = new Point2D.Double(pRadial.getX() + gvWidth / 2,
                                                  pRadial.getY());
