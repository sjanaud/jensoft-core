...
public class V1Needle extends NeedleGaugePainter {
    ...
    public void setCurentValue(int value) {
        ...
        if (value < getGauge().getMetricsManager().getMin()) {
            throw new IllegalArgumentException(
                                               "gauge needle value should be greater that min value affect to the metrics path.");
        }
        if (value > getGauge().getMetricsManager().getMax()) {
            throw new IllegalArgumentException(
                                               "gauge needle value should be smaller that max value affect to the metrics path.");
        }
    ...
    private void paintNeedle(Graphics2D g2d) {
        ...
        BasicStroke stroke2 = new BasicStroke(8, BasicStroke.CAP_ROUND,
                                              BasicStroke.JOIN_ROUND);
        ...
        g2d.setColor(ColorPalette.alpha(JennyPalette.JENNY10, 120));

        g2d.fill(area2);

        g2d.setColor(JennyPalette.JENNY10);
        ...
        RadialGradientPaint rgp = new RadialGradientPaint(new Point2D.Double(
                                                                             centerX, centerY), 20, new float[] { 0f,
                1f }, new Color[] {
                Color.BLACK, Color.BLACK.darker() });
