...
public class ClassicEnvelop extends EnvelopGaugePainter {
	 ...
	 * 
	 * @see
	 * com.jensoft.core.plugin.gauge.core.env.EnvelopGaugePainter#paintEnvelop
	...
	public void paintEnvelop(Graphics2D g2d, RadialGauge radialGauge) {
		...
		if (envelopPart == null) {
			...
			envelopPart = new PartBuffer(centerX - radius / 2, centerY - radius / 2, 2 * radius, 2 * radius);
			...
			g2dPart.drawImage(envelopPart.getBuffer(), (int) centerX - radius, (int) centerY - radius, 2 * radius, 2 * radius, null);

		} else {
			// System.out.println(" paint from cache ENVELOP");
			g2d.drawImage(envelopPart.getBuffer(), (int) centerX - radius, (int) centerY - radius, 2 * radius, 2 * radius, null);
		}
