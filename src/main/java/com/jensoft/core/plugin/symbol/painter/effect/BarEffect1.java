...
public class BarEffect1 extends AbstractBarEffect {
    ...
    public void paintBarEffect(Graphics2D g2d, BarSymbol bar) {
        if (bar.getNature() == SymbolNature.Vertical) {
            paintEffectVBar_ef1(g2d, bar);
        }
        if (bar.getNature() == SymbolNature.Horizontal) {
            paintEffectHBar_ef1(g2d, bar);
        }
    }

    private void paintEffectVBar_ef1(Graphics2D g2d, BarSymbol bar) {
        ...
        if (bar.getMorpheStyle() == MorpheStyle.Round) {
            ...
            Rectangle2D barRec = new Rectangle2D.Double(x, y, width / 2, height);
            shapeEffect = barRec;
        }
    ...
    private void paintEffectHBar_ef1(Graphics2D g2d, BarSymbol bar) {
        ...
        Point2D end = null;
        if (bar.isAscent()) {
            start = new Point2D.Double(boun2D2.getX(), boun2D2.getCenterY());
            end = new Point2D.Double(boun2D2.getX() + boun2D2.getWidth(),
                                     boun2D2.getCenterY());
        }
        else if (bar.isDescent()) {
            start = new Point2D.Double(boun2D2.getX() + boun2D2.getWidth(),
                                       boun2D2.getCenterY());
            end = new Point2D.Double(boun2D2.getX(), boun2D2.getCenterY());
        }
        float[] dist = { 0.0f, 1.0f };
        Color[] colors = { new Color(255, 255, 255, 120),
                new Color(255, 255, 255, 80) };
