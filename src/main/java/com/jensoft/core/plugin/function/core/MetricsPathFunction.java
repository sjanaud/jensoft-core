...
import com.jensoft.core.glyphmetrics.Marker;
...
public class MetricsPathFunction extends AbstractMetricsPath {
    ...
    /** the source for this metrics path function */
     ...
     * <p>
     * set the source source for this metrics path function and make a request to solve the geometry on the next demand.
     * 
     * @param source
    ...
    class PathSegment {
         ...
         */
        public PathSegment(Point2D segmentUserStart, Point2D segmentUserEnd,
        ...
        /**
         * return true if segment x range contains specified user x value, false otherwise
         * 
         * @param userX
         *            the user x coordinate to test
         * @return true if segment x range contains specified user x value, false otherwise
         */
        ...
        /**
         * <p>
         * get A slope coefficient of this segment
         * </p>
         * <p>
         * y = Ax + B
         * </p>
        ...
        public String toString() {
            return "PathSegment [segmentUserStart=" + segmentUserStart
    ...
    /**
     * get the curve segment that contains the specified x in user projection system
     * 
     * @param userX
     *            the x value in user projection
     * @return the curve segment that contains the specified x in user projection system
     */
    ...
    protected GeneralPath createPathMetrics() {
        ...
        List<Point2D> entries = source.select(getWindow2d().getMinX(), getWindow2d().getMaxX());
        ...
        for (int i = 0; i < src.size(); i++) {
            ...
            if (i == 0) {
                ...
                Point2D segmentDeviceStart = getWindow2d().userToPixel(
                                                                       segmentUserStart);

                PathSegment cs = new PathSegment(segmentUserStart, serieItem,
                                                 segmentDeviceStart, segmentDeviceEnd);
                pathSegments.add(cs);
    ...
    /**
     * get radial point for the specified user value at the specified side of this metrics path
     * 
     * @param userValue
     *            the user value
     * @param radius
     ...
     * @param side
     *            teh path side
     * @return radial point for the specified user value at the specified side of this metrics path
     */
    ...
    public List<GlyphMetric> getMetrics() {
        ...
        for (GlyphMetric vm : volatileMetrics) {
            ...
            m.setMetricGlyphMarker(new Marker(getOrCreateGeometry()
            ...
            if (m.getStylePosition() == StylePosition.Tangent) {
                ...
                for (int j = 0; j < glyphVector.getNumGlyphs(); j++) {
                    ...
                    if (!needRevert) {
                        glyphPoint = getOrCreateGeometry().pointAtLength(startLength
                                + GlyphUtil
                                        .getGlyphWidthAtToken(glyphVector, j));
                    }
                    else {
                        glyphPoint = getOrCreateGeometry().pointAtLength(endLength
                                - GlyphUtil
                                        .getGlyphWidthAtToken(glyphVector, j));
                    }
                    ...
                    float angle = 0;
                    if (!needRevert) {
                        angle = getOrCreateGeometry().angleAtLength(startLength
                                + GlyphUtil
                                        .getGlyphWidthAtToken(glyphVector, j));
                    }
                    else {
                        angle = getOrCreateGeometry().angleAtLength(endLength
                                - GlyphUtil
                                        .getGlyphWidthAtToken(glyphVector, j));
                    }
            ...
            if (m.getStylePosition() == StylePosition.Default) {
                ...
                if (pRadial != null) {
                    ...
                    for (int j = 0; j < glyphVector.getNumGlyphs(); j++) {
                        ...
                        Shape glyphTransformed = af
                                .createTransformedShape(glyph);

                        // new with glyphgeometry
                        Point2D srcNorth = new Point2D.Double(glyph
                                .getBounds2D().getCenterX(), glyph
                                .getBounds2D().getY());
                        Point2D dstNorth = new Point2D.Double();

                        Point2D srcSouth = new Point2D.Double(glyph
                                .getBounds2D().getCenterX(), glyph
                                .getBounds2D().getY()
                                + glyph.getBounds2D().getHeight());
                        Point2D dstSouth = new Point2D.Double();

                        Point2D srcEast = new Point2D.Double(glyph
                                .getBounds2D().getX()
                                + glyph.getBounds2D().getWidth(), glyph
                                .getBounds2D().getCenterY());
                        Point2D dstEast = new Point2D.Double();

                        Point2D srcWest = new Point2D.Double(glyph
                                .getBounds2D().getX(), glyph.getBounds2D()
                                .getCenterY());
                        Point2D dstWest = new Point2D.Double();
