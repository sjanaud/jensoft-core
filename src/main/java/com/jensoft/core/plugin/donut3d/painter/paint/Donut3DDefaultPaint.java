...
public class Donut3DDefaultPaint extends AbstractDonut3DPaint {
	...
	public void paintDonut3DFill2(Graphics2D g2d, Donut3D donut3d, Donut3DSlice s) {
		 ...
		 */
		g2d.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, alphaFill));
	...
	public void paintTopEffect(Graphics2D g2d, Donut3D donut3d, Donut3DSlice section) {
		...
		g2d.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, alphaTop));
		...
		// tilt 0--> 90
		// start Section
		Donut3DSlice startSection = donut3d.getSliceOnAngle(incidenceAngleDegree);
		...
		double centerStartX = centerX + exploseStartA * Math.cos(Math.toRadians(startSection.getStartAngleDegree() + startSection.getExtendsDegree() / 2));
		double centerStartY = centerY - exploseStartB * Math.sin(Math.toRadians(startSection.getStartAngleDegree() + startSection.getExtendsDegree() / 2));

		double startX = centerStartX + donut3d.getOuterA() * Math.cos(Math.toRadians(incidenceAngleDegree));
		double startY = centerStartY - donut3d.getOuterB() * Math.sin(Math.toRadians(incidenceAngleDegree));

		// end section

		Donut3DSlice endSection = donut3d.getSliceOnAngle(incidenceAngleDegree + 180);
		if (endSection == null) {
			return;
		}
		...
		double centerEndX = centerX + exploseEndA * Math.cos(Math.toRadians(endSection.getStartAngleDegree() + endSection.getExtendsDegree() / 2));
		double centerEndY = centerY - exploseEndB * Math.sin(Math.toRadians(endSection.getStartAngleDegree() + endSection.getExtendsDegree() / 2));

		double endX = centerEndX + donut3d.getOuterA() * Math.cos(Math.toRadians(incidenceAngleDegree + 180));
		double endY = centerEndY - donut3d.getOuterB() * Math.sin(Math.toRadians(incidenceAngleDegree + 180));
	...
	public void paintOuterEffect(Graphics2D g2d, Donut3D donut3d, Donut3DSlice section) {
		g2d.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, alphaOuter));
	...
	public void paintInnerEffect(Graphics2D g2d, Donut3D donut3d, Donut3DSlice section) {
		g2d.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, alphaInner));
		...
		Color[] colorsInner = { cStartInner, cStartInnerbis, cEndInnerbis, cEndInner };
