 ...
 */
public class ZoomObjectifPlugin extends AbstractPlugin implements
        ...
        synchronized (listeners) {
            for (int i = 0; i < listeners.length; i += 2) {
                if (listeners[i] == ZoomObjectifListener.class) {
                    ((ZoomObjectifListener) listeners[i + 1])
                            .zoomIn(new ZoomObjectifEvent(this));
                }
            }
        }
    }
    ...
    public void fireZoomOut() {
        ...
        synchronized (listeners) {
            for (int i = 0; i < listeners.length; i += 2) {
                if (listeners[i] == ZoomObjectifListener.class) {
                    ((ZoomObjectifListener) listeners[i + 1])
                            .zoomOut(new ZoomObjectifEvent(this));
                }
            }
        }
    }
    ...
    public void lockSelected() {
        ...
        getWindow2D().getView2D().getWidgetPlugin()
                .pushMessage("LOCK OBJECTIF", this, PushingBehavior.Fast);
    ...
    public void startZoomOut(ZoomNature zoomNature) {
        zoomOutLock = true;
        Thread zoomOutThread = new Thread(getZoomOutProcess(zoomNature),
                                          "zoom out run");
        zoomOutThread.start();
    }
    ...
    public void startZoomIn(ZoomNature zoomNature) {
        zoomInLock = true;
        Thread zoomInThread = new Thread(getZoomInProcess(zoomNature),
                                         "zoom in run");
        zoomInThread.start();
    }
