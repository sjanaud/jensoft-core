...
public class RayDefaultAxisLabel extends AbstractRayAxisLabel {
     ...
     */
    public RayDefaultAxisLabel(String symbol, Color symbolColor, int offsetX,
            int offsetY) {
        super();
        this.symbol = symbol;
        this.symbolColor = symbolColor;
     ...
     *            the window part
     */
    private void paintXRayAxisLabel(Graphics2D g2d, Ray ray,
        ...
        if (windowPart == WindowPart.South) {
            ...
            if (ray instanceof Ray && !(ray instanceof RayGroup)) {
                ...
                if (symbol != null) {
                    ...
                    float pt = (float) (ray.getRayShape().getX() + margin
                            + barWidth / 2 - symbolWidth / 2);

                    int x = (int) (pt + offsetX);
                    int y = fm.getAscent() + offsetY;

                    RoundRectangle2D roundRect = new RoundRectangle2D.Double(x
                            - internalMarginX, y - fm.getAscent()
                            - internalMarginY, symbolWidth + 2
                            * internalMarginX, fm.getHeight() + 2
                            * internalMarginY, round, round);
     ...
     *            the window part
     */
    private void paintYRayAxisLabel(Graphics2D g2d, Ray bar,
            WindowPart windowPart) {
        ...
        if (windowPart == WindowPart.West) {
            ...
            if (bar instanceof Ray && !(bar instanceof RayGroup)) {
                ...
                if (symbol != null) {
                    ...
                    int symbolWidth = fm.stringWidth(symbol);
                    float pt = (float) (bar.getRayShape().getY() + barHeight
                            / 2 + fm.getAscent() / 2);
