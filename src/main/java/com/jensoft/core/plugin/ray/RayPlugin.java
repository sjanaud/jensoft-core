...
public class RayPlugin extends AbstractPlugin implements AbstractPlugin.OnClickListener, AbstractPlugin.OnEnterListener, AbstractPlugin.OnExitListener, AbstractPlugin.OnPressListener, AbstractPlugin.OnReleaseListener, AbstractPlugin.OnMoveListener, AbstractPlugin.OnDragListener {
	...
	private void resolveRayGeometry(Ray ray) {
		...
		if (ray.getRayNature() == RayNature.XRay) {
			...
			double centerDeviceX = w2d.userToPixel(new Point2D.Double(centerUserX, 0)).getX();
			...
			double yDeviceRayBase = w2d.userToPixel(new Point2D.Double(0, yUserRayBase)).getY();
			...
			double yDeviceRayFleche = w2d.userToPixel(new Point2D.Double(0, yUserRayFleche)).getY();
			...
			double centerDeviceY = w2d.userToPixel(new Point2D.Double(0, centerUserY)).getY();
			...
			double xDeviceRayBase = w2d.userToPixel(new Point2D.Double(xUserRayBase, 0)).getX();
			...
			double xDeviceRayFleche = w2d.userToPixel(new Point2D.Double(xUserRayFleche, 0)).getX();
	...
	private void resolveStackedRayGeometry(StackedRay stackedRay) {
		...
		if (stackedRay.getRayNature() == RayNature.XRay) {
			...
			for (RayStack s : stackedRay.getStacks()) {
				...
				double yDeviceStackRayBase = w2d.userToPixel(new Point2D.Double(0, yUserStackRayBase)).getY();
				...
				double yDeviceStackRayFleche = w2d.userToPixel(new Point2D.Double(0, yUserStackRayFleche)).getY();
			...
			for (RayStack s : stackedRay.getStacks()) {
				...
				double xDeviceStackRayBase = w2d.userToPixel(new Point2D.Double(xUserStackRayBase, 0)).getX();
				...
				double xDeviceStackRayFleche = w2d.userToPixel(new Point2D.Double(xUserStackRayFleche, 0)).getX();
	...
	public void onRelease(MouseEvent me) {
		for (Ray ray : rays) {
			if (ray.getRayShape() != null && ray.getRayShape().contains(me.getX(), me.getY())) {
				fireRayReleased(ray);
			}
		}
	}
