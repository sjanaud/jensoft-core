...
public class FlowMetricsManager extends AbstractMetricsManager {
     ...
     */
   // @Override
    public List<Metrics> getDeviceMetrics2() {
        deviceMetrics.clear();
        Projection projection = getMetricsPlugin().getProjection();
        BigDecimal start = new BigDecimal(flowStart+"");
        BigDecimal end = new BigDecimal(flowEnd+"");
        BigDecimal inter = new BigDecimal(flowInterval+"");
        
       //double interval = flowEnd - flowStart;

        int metricsCount = (int) ((flowEnd - flowStart) / flowInterval);
        ...
        if (metricsCount > 0) {
            for (int i = 0; i <= metricsCount; i++) {
                ...
                if (getType() == MetricsType.XMetrics) {
                    if (userMetricsValue >= projection.getMinX()  && userMetricsValue <= projection.getMaxX()) {
                          
                        Point2D p2dUser = new Point2D.Double(userMetricsValue, 0);
                        Point2D p2dDevice = projection.userToPixel(p2dUser);
                        ...
                        if (getMetricsFormat() == null) {
                            metrics.setMetricsLabel(getDefaultFormat().format(userMetricsValue));
                        }
                        else {
                            metrics.setMetricsLabel(getMetricsFormat().format(userMetricsValue));
                        }
                ...
                if (getType() == MetricsType.YMetrics) {
                    if (userMetricsValue >= projection.getMinY()  && userMetricsValue <= projection.getMaxY()) {
                          
                        Point2D p2dUser = new Point2D.Double(0,userMetricsValue);
                        Point2D p2dDevice = projection.userToPixel(p2dUser);
