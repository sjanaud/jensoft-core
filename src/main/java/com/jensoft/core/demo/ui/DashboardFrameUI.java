...
public class DashboardFrameUI extends JFrame {
	...
	private void loadViewSource() {
		...
		String inputSourceName = "NA";
		try {
			ClassLoader cloader = demo.getClass().getClassLoader();
			String packageName = demo.getClass().getPackage().getName();
			inputSourceName = packageName.replace('.', '/') + "/" + demo.getClass().getSimpleName() + ".java";
