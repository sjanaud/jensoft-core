...
//        registerInflater(new AxisMetricsInflater.MultiplierMetricsInflater());
//        registerInflater(new AxisMetricsInflater.MultiMultiplierMetricsInflater());
//
//        // grids
...
//        registerInflater(new TranslateInflater());
    ...
    public String getAPIKey() {
        Element e = (Element) viewRoot.getElementsByTagName(ELEMENT_API_KEY)
                .item(0);
     ...
     */
    public int getWidth() {
        Element e = (Element) viewRoot.getElementsByTagName(ELEMENT_VIEW_WIDTH)
                .item(0);
    ...
    public int getHeight() {
        Element e = (Element) viewRoot
                .getElementsByTagName(ELEMENT_VIEW_HEIGHT).item(0);
    ...
    protected AbstractX2DPluginInflater<?> lookupTypeClass(String xsiType, String typeClass) {
        for (AbstractX2DPluginInflater<?> inflater : inflaters) {
            if (inflater.getXSIType() != null && inflater.getXSIType().equals(xsiType)
                    && inflater.getPlugin().getClass().getName().equals(typeClass)) {
                return inflater;
            }
        }
        return null;
    ...
    public BackgroundPainter parseBackground(Element background) {
        ...
        try {
            ...
            String outlineround = background
                    .getElementsByTagName(ELEMENT_VIEW_BACKGROUND_OUTLINEROUND)
                    .item(0).getTextContent();

            Element outlinecolorElement = (Element) background
                    .getElementsByTagName(ELEMENT_VIEW_BACKGROUND_COLOR)
                    .item(0);

            Element shaderElement = (Element) background.getElementsByTagName(
                                                                              ELEMENT_VIEW_BACKGROUND_SHADER).item(0);
            Element strokeElement = (Element) background.getElementsByTagName(
                                                                              ELEMENT_VIEW_BACKGROUND_STROKE).item(0);
            ...
            roundView.setOutlineColor(InflaterUtil
                    .elementColor(outlinecolorElement));
    ...
    private View2D generateView2D() throws X2DException {
        ...
        NodeList windowsElements = viewRoot
                .getElementsByTagName(ELEMENT_VIEW_WINDOW2D);
        for (int i = 0; i < windowsElements.getLength(); i++) {
            ...
            try {
                ...
                NodeList pluginsElements = w2dElement
                        .getElementsByTagName(ELEMENT_VIEW_PLUGIN);
                for (int j = 0; j < pluginsElements.getLength(); j++) {
                    ...
                    if (inflater != null) {
                        inflater.inflate(pluginElement);
                        inflater.getPlugin().setPluginID(pluginID);
                        inflater.getPlugin().setName(pluginName);
                        w2d.registerPlugin(inflater.getPlugin());
                    }
                    else {
                        if (pluginClass == null) {
                        ...
                        if (pluginClass != null) {
                            System.err.println("Plugin inflater type not found : " + pluginType + " for plugin class "
                                    + pluginClass + "\n");
                            throw new X2DException("Plugin inflater type not found : " + pluginType
                                    + " for plugin class "
                                    + pluginClass + "\n");
