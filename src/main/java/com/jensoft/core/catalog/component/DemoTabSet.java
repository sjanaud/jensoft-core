...
public class DemoTabSet extends JComponent implements DemoTabListener {
	...
	public Insets getInsets() {
		...
		insetComandBar = new Insets(sheigh + verticalOffset + baseLineOffset + 6, 6, 16, 6);
	...
	protected void paintComponent(Graphics g) {
		...
		for (int i = 0; i < commands.size(); i++) {
			...
			tabPath.quadTo(c.getStartX() - 10, baseLineTab, c.getStartX() - 10, baseLineTab - 5);
			tabPath.lineTo(c.getStartX() - 10, baseLine - sheigh);
			tabPath.quadTo(c.getStartX() - 10, baseLine - sheigh - 10, c.getStartX(), baseLine - sheigh - 10);
			tabPath.lineTo(c.getStartX() + c.getEndX(), baseLine - sheigh - 10);
			tabPath.quadTo(c.getStartX() + c.getEndX() + 10, baseLine - sheigh - 10, c.getStartX() + c.getEndX() + 10, baseLine - sheigh);
			...
			tabPath.quadTo(getWidth() - 6, getHeight() - 6, getWidth() - 16, getHeight() - 6);
			tabPath.closePath();
			...
			int innerMargin = 4;
			tabSensiblePath.moveTo(c.getStartX() - 10 + innerMargin, baseLineTab - 5);

			tabSensiblePath.lineTo(c.getStartX() - 10 + innerMargin, baseLine - sheigh);
			tabSensiblePath.quadTo(c.getStartX() - 10 + innerMargin, baseLine - sheigh - 10 + innerMargin, c.getStartX(), baseLine - sheigh - 10 + innerMargin);
			tabSensiblePath.lineTo(c.getStartX() + c.getEndX(), baseLine - sheigh - 10 + innerMargin);
			tabSensiblePath.quadTo(c.getStartX() + c.getEndX() + 10 - innerMargin, baseLine - sheigh - 10 + innerMargin, c.getStartX() + c.getEndX() + 10 - innerMargin, baseLine - sheigh);
			tabSensiblePath.lineTo(c.getStartX() + c.getEndX() + 10 - innerMargin, baseLineTab - 5);
			...
			CubicCurve2D curve = new CubicCurve2D.Double(getWidth() - 6, getHeight() / 2, getWidth() / 2, baseLineTab, getWidth() / 2, getHeight(), 5, getHeight() / 2);
		...
		RoundRectangle2D roundBaseShape = new RoundRectangle2D.Double(0, 0, getWidth() - 1, getHeight() - 1, cornerRadius, cornerRadius);

		GradientPaint gpbase = new GradientPaint(getWidth() / 2, 0, Color.LIGHT_GRAY, getWidth() / 2, baseLine, Color.GRAY, false);
		...
		for (int j = 0; j < commands.size(); j++) {
			...
			GradientPaint gp = new GradientPaint(getWidth() / 2, baseLine - sheigh - 10, Color.WHITE, getWidth() / 2, baseLineTab + 5, c.getTabColor(), false);
			g2d.setPaint(gp);
			...
			g2d.drawString(c.getComandName(), c.getStartX() + c.getIconWidth(), baseLine - 5);
			if (c.getTabIcon() != null) {
				g2d.drawImage(c.getTabIcon().getImage(), c.getStartX(), baseLine - c.getTabIcon().getIconHeight() - 2, c.getTabIcon().getIconWidth(), c.getTabIcon().getIconHeight(), null);
			}

		}

		if (selectedTab != null) {
			GradientPaint gp = new GradientPaint(getWidth() / 2, 20, Color.WHITE, getWidth() / 2, getHeight() / 2, selectedTab.getTabColor(), false);
			g2d.setPaint(gp);
			...
			g2d.setColor(Color.DARK_GRAY);
			g2d.drawString(selectedTab.getComandName(), selectedTab.getStartX() + selectedTab.getIconWidth(), baseLine - 5);
			if (selectedTab.getTabIcon() != null) {
				g2d.drawImage(selectedTab.getTabIcon().getImage(), selectedTab.getStartX(), baseLine - selectedTab.getTabIcon().getIconHeight() - 2, selectedTab.getTabIcon().getIconWidth(), selectedTab.getTabIcon().getIconHeight(), null);
			}
