 ...
 * plane. It is distinct from <code>Point</code>, which is a subclass of <code>Geometry</code> . Unlike objects of type
 * <code>Point</code> (which
 ...
 * <P>
 * <code>Coordinate</code>s are two-dimensional points, with an additional z-ordinate. JTS does not support any
 * operations on the z-ordinate except the basic accessor functions. Constructed coordinates will have a z-ordinate of
 * <code>NaN</code>. The standard comparison functions will ignore the z-ordinate.
 * 
 * @version 1.7
 */
public class Coordinate implements Comparable, Cloneable, Serializable {
     ...
     * @return <code>true</code> if <code>other</code> is a <code>Coordinate</code> with the same values for x, y and z.
     */
